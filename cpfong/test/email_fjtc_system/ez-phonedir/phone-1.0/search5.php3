...
$sql="SELECT * FROM address WHERE lower(birthday) LIKE lower('$name%') order by last;";
$sql2="SELECT count(*) from address WHERE lower(birthday) LIKE lower('$name%');";
...
if ((!$result_set) || ($rows < 1)) {
       ...
       echo "<H1>Sorry no birthdays for $name was not found. Please redo your search. </H1><P>";
       echo "<table>";
...
echo "<tr><th align=left><a href=search7.php3?name=$pos16>View Full Record</a></th><td align=left>$pos $pos2 </td><td> Birthday: $pos13 </td><td> Phone: ( $pos18 ) $pos3</td><td> &nbsp Email: &nbsp $pos11</td></tr>";
 }
}
?>
