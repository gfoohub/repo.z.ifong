...
$sql="SELECT * FROM address WHERE lower(state) LIKE lower('$name%') order by last;";
$sql2="SELECT count(*) from address WHERE lower(state) LIKE lower('$name%');";
...
if ((!$result_set) || ($rows < 1)) {
       ...
       echo "<H1>Sorry the listing for State $name was not found. Please redo your search. </H1><P>";
       echo "<table>";
...
echo "<tr><th align=left><a href=search7.php3?name=$pos16>View Full Record</a></th><td align=left>$pos $pos2 </td><td> Phone:&nbsp( $pos18 ) $pos3</td><td> &nbsp Address: &nbsp $pos4 $pos5 $pos6 $pos7</td></tr>";
 }
}
?>
